...
Definition greater (n m : nat) : bool := negb (beq_nat (minus n m) O).
Definition geq (n m : nat) : bool := orb (greater n m) (beq_nat n m).

Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => (evenb x) && greater x 7) l.
...
Theorem map_app : forall (X Y : Type) (f : X -> Y) (xs : list X) (ys : list X),
...
(* A fold that checks whether all elements in an int list are even 
would use X: int and Y: bool.*)
(** [] *)
...
(** Recall the definition of the [nth_error] function:
   ...
   Proof:
   We prove that n can never be zero while there are remaining 
   elements in the list by induction.

   First, suppose that l is nil.
   Then regardless of n, there can be no element to return.

   Suppose then that l is x :: xs.
   Since the length is now nonzero by definition, 
   the recursive call is always followed. 
   The condition that length l = n is maintained, as the recursion both removes
   an element from the list and decrements n by one. 
   By the induction hypothesis, nth_error X xs (n-1) is None.
...
Definition exp (n m : nat) : nat :=
  fold (fun _ state => mult n state) (m (list unit) (cons tt) []) one.
(*  *)
