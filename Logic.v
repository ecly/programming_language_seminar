...
(** Write an informal proof of [double_neg]:
   ...
   _Theorem_: [P] implies [~~P], for any proposition [P].
    ...
    Proof: Since the definition of ~P is that P implies False, ~P can 
    be rewritten as (P -> False) and ~~P therefore (P -> False) -> False.
    
    Since we have P and are assuming that P -> False, we are also
    assuming False. We therefore have a proof by contradiction.
    Qed.
*)
(** [] *)
...
(** Write an informal proof (in English) of the proposition [forall P
    ...
    Proof: Rewriting _not P_ to its definition (P -> False), we get that
    P and (P implies False) implies False. Splitting the assumption into 
    its parts, we get both P and P implies False. 
    Since our goal is False, and P implies it, we only need to prove P.
    But since P is assumed, we are done.
    Qed.
*)
(** [] *)
...
Proof.
  split.
  - intros. destruct H as [x E]. inversion E. 
    + left. exists x. apply H.
    + right. exists x. apply H.
  - intros. inversion H. 
    + destruct H0 as [x E]. exists x. left. apply E.
    + destruct H0 as [x E]. exists x. right. apply E.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. split. 
    ...
    + intros H. destruct H. inversion H. inversion H1.
  - simpl. rewrite IHl. split. 
    + intros. destruct H. 
      * exists x. split. { apply H. } { left. reflexivity. }
      * destruct H as [x' E]. destruct E. exists x'. split. 
        { apply H. } { right. apply H0. }
    + intros. destruct H as [x' E]. destruct E. destruct H0. 
      * left. rewrite <- H0 in H. apply H.
      * right. exists x'. split. { apply H. } { apply H0. }
Qed. 
(** [] *)
...
Proof.
  ...
  - simpl. split. 
    ...
    + intros. rewrite IHl. rewrite <- or_assoc in H. apply H.
Qed.
(** [] *)
